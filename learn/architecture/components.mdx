...
## Onchain Anchoring via Batch Settlements

Arkade minimizes onchain footprint by batching user activity and anchoring it to Bitcoin through periodic **Batch Settlements**. Instead of each user creating individual onchain outputs, multiple users are aggregated into a single onchain transaction known as a **Commitment Transaction**. This transaction settles the batch onchain by creating a **Batch Output** that represents all users' ownership claims through a virtual transaction tree.
