...
When a user wants to settle their funds onchain via a [batch swap](/learn/ark/batch-swaps), they submit an [intent](/learn/architecture/intents) to the operator. Arkade intents are valid-but-unmineable Bitcoin transactions that encode an ownership proof of the inputs a user wants to redeem - whether it's offchain VTXOs, an onchain UTXO or [recoverable coins](/learn/ark/vtxo-lifecycle) (eg. expired coins or in form of an [Arkade Note](/arkd/components/ark-notes)) - and define the outputs they wish to receive.
...
<Tip>
  The usage of BIP322 enables users to choose between renewing their [expiring](/learn/ark/tx-batching#batch-expiry) offchain funds themselves or [delegating](/learn/architecture/delegation) the renewal of such VTXOs. The delegated intent workflow can be found under [Intent Delegation](/arkd/components/intent-delegation#intent-delegation-workflow)
</Tip>
...
The intent system provides additional options for edge cases like [recoverable VTXOs](/learn/ark/vtxo-lifecycle):
