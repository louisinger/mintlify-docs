...
<AccordionGroup>
  ...
  <Accordion title="Forfeit Management">
    `SubmitSignedForfeitTxs` handles [forfeit transaction](/learn/ark/batch-swaps#forfeit-transactions) submission. The Arkade server verifies and finalizes the transaction as part of the batch settlement process.
<Note>If a [delegate](/learn/architecture/delegation) is involved, the forfeit transaction is partially signed with `SIGHASH_ALL | ANYONECANPAY` allowing  a [connector output](/learn/ark/tx-batching#connector-output) to be added once the delegate submits the intent.</Note>

    
  </Accordion>
  <Accordion title="Offchain Execution">
    Arkade’s [virtual mempool](/learn/architecture/virtual-mempool) tracks the two-phase lifecycle of [offchain transactions that spend VTXOs](/arkd/txs/offchain-execution):
