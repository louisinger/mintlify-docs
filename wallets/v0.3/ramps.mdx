...
description: "Mechanisms for boarding onto and exiting from Arkade"
icon: "bitcoin"
---

Ramps are the tools that allow users with Bitcoin to onboard onto and exit from Arkade with its own built-in mechanisms. They are designed to provide a seamless experience for users, allowing them to move funds in and out of Arkade. There are two types of ramps: Boarding (onboarding) and Exiting (offboarding).
...
The Arkade Operator processes the registration and includes the UTXO in a [Commitment Transaction](/learn/pillars/batch-swaps#commitment-transactions).

### 5. Receive a VTXO

Once the Commitment Transaction is confirmed, you now have a VTXO within a Batch Output that you can spend offchain.
...
Unilateral exit requires [unrolling a set of transactions](/learn/security/unilateral-exit#exit-requirements) leading to your VTXO. Each transaction includes a P2A output that covers the miner fees. So you need onchain funds to pay for them.
...
const session = await Unroll.Session.create(
  ...
  wallet.indexerProvider // the [Arkade Indexer](/arkd/core-services/indexer-service) to fetch virtual transactions
);
...
await Unroll.completeUnroll(
  wallet, // the Arkade wallet owning the VTXO
